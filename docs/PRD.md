...
## 2.5 사용자 시나리오 (User Scenarios & Journey)

### 🧑‍💻 시나리오 A: 소유자(Owner / Creator)의 여정
> **"나만의 감각적인 멀티 링크를 1분 만에 완성하고 SNS에 공유하는 민준의 경험"**

1. **탐색 및 시작**:
   - 마이링크 랜딩 페이지(`/`)에 접속하여 **"30초 만에 시작하기"** 버튼을 클릭합니다.
2. **프로필 커스텀**:
   - 관리자 대시보드(`/admin`)에서 닉네임 `김민준`, 아이디 `@mlm577373`, 한 줄 소개를 입력합니다.
   - 텍스트를 입력하는 즉시 **우측 가상 스마트폰 화면(실시간 목업)에 딜레이 없이 반영**되는 것을 확인합니다.
3. **스마트 링크 등록**:
   - `+ 새 링크 추가` 버튼을 눌러 GitHub 주소(`https://github.com/mlm577373`)를 입력합니다.
   - URL을 입력하자마자 시스템이 자동으로 **GitHub 고유 아이콘과 브랜드 컬러를 감지**하여 지정해 주는 편리함을 경험합니다.
   - 이어서 유튜브, 이메일, 포트폴리오 링크도 추가합니다.
4. **정렬 및 노출 관리**:
   - 가장 알리고 싶은 "포트폴리오" 카드를 위쪽으로 순서 이동합니다.
   - 아직 준비 중인 "기술 블로그" 링크는 **토글 스위치를 꺼서 임시 비공개(숨김)** 상태로 둡니다.
5. **테마 스타일링**:
   - 테마 프리셋 탭에서 **'Aurora Dream'** 테마를 선택하자 우측 스마트폰 화면이 세련된 핑크-퍼플 그라데이션과 글래스모피즘으로 변신합니다.
   - 버튼 모양을 둥근 알약형(`Pill`)으로 선택해 나만의 디자인을 완성합니다.
6. **공유 및 실시간 통계 확인**:
   - 상단의 **"내 링크 복사 (`mylink/mlm577373`)"**를 눌러 인스타그램과 GitHub 바이오에 등록합니다.
   - 며칠 후 대시보드에 다시 들어와 방문자들이 어떤 링크를 가장 많이 클릭했는지 **실시간 누적 클릭 수 통계**를 확인합니다.

---

### 👀 시나리오 B: 방문자(Visitor)의 여정
> **"SNS 링크를 타고 들어와 매끄럽게 콘텐츠를 탐색하고 공유하는 방문자 수진의 경험"**

1. **유입 (Entry)**:
   - 인스타그램 프로필이나 포트폴리오 이력서에 적힌 링크(`mylink/mlm577373`)를 스마트폰으로 터치합니다.
2. **첫인상 (First Impression)**:
   - 0.5초 만에 제작자가 세팅한 감각적인 오로라 테마의 모바일 프로필 페이지(`/[username]`)가 로딩됩니다.
   - 상단의 아바타와 닉네임, 그리고 매력적인 한 줄 소개를 읽으며 제작자의 전문성과 정체성을 직관적으로 파악합니다.
3. **탐색 및 상호작용 (Engagement)**:
   - 깔끔하게 정돈된 링크 카드 목록을 살펴보다 관심 있는 **"GitHub 프로젝트"**와 **"포트폴리오"** 카드를 발견합니다.
   - 카드를 터치하자 부드러운 스케일 애니메이션과 함께 새 창으로 원하는 사이트가 안전하게 열립니다. (백그라운드에서 클릭 카운트 1 증가)
4. **공유 및 바이럴 (Share & Viral)**:
   - 우측 상단 **"공유하기"** 버튼을 눌러 모바일 공유 시트 또는 링크 복사로 지인에게 링크를 전달합니다.
   - 하단에 은은하게 위치한 **"나만의 마이링크 만들기"** 배너를 보고 *"나도 내 링크트리 하나 만들어볼까?"* 호기심을 갖고 서비스 랜딩 페이지로 이동합니다.
